...
Convert_csv_to_multinet_multiplex = function(nodes, links){
	...
	layer_index  = distinct(links, layer)
  	...
  	# Name of output file
  	outfile <- "random/outfile_random.txt"
...
Convert_to_Multinet <- function(nodes, links){
  	...
  	net_multinet = read_ml("random/outfile_random.txt", name = "unnamed", sep = ',', aligned = FALSE) #import "outifile.txt" to a multinet type network
  	return(net_multinet)
	}#end function
